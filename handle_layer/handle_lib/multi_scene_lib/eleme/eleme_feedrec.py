...
class elemeFeedrec(InputBase):
    def __init__(self, data_struct, params, is_training, hashtable, ptable_lookup):
        super(elemeFeedrec, self).__init__(data_struct, params, is_training, hashtable, ptable_lookup)
    ...
    def __call__(self, cat_features, dense_features, auxiliary_info, embed_dim, se_block):
        ...
        ## feedrec
        split_list = self.seq_split(tf.expand_dims(self.item_info_list_emb[:, 0], 1), self.shop_list_emb, self.time_type_list, [1442745, 898382, 15195206, 8572009, 11903286], 'time_type') # 3284598
        split_list_stack = tf.stack(split_list, 1)
        split_list_weight = tf.get_variable('split_list_weight', shape=[1, len(split_list)], initializer=tf.random_normal_initializer())
        feed_rec = tf.matmul(split_list_weight, split_list_stack) # [B, 1, 8]
        mask = tf.count_nonzero(self.dense_seq_fea, -1, keep_dims=True)
        feed_output = self.attention_layer(feed_rec, self.shop_list_emb, mask,
                                            embed_dim, 1, self.seq_len,
                                            self.params['din_deep_layers'], self.params['din_activation'],
                                            'eleme_feedrec', 'eleme_feedrec')

        output = tf.concat([feed_output, tf.squeeze(feed_rec, 1), output_dense, tf.layers.flatten(self.user_info_list_emb), tf.layers.flatten(self.item_info_list_emb)], -1)
        return output
        ...
        # output = tf.concat([output_dense, output_din, tf.layers.flatten(self.user_info_list_emb), tf.layers.flatten(self.item_info_list_emb)], -1)
        # return output
    ...
    def seq_split(self, target, input_seq, scene_id_seq, scene_id_list, name):
        with tf.variable_scope('seq_split_' + name, reuse=tf.AUTO_REUSE):
            ...
            for i in range(len(scene_id_list)):
                att = self.my_dense(din_input, 1, name='seq_split_%d'%i, activation=tf.nn.sigmoid) # [B, seq_len, 1]
