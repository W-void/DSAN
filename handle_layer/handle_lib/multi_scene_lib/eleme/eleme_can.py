...
class elemeCan(InputBase):
    def __init__(self, data_struct, params, is_training, hashtable, ptable_lookup):
        super(elemeCan, self).__init__(data_struct, params, is_training, hashtable, ptable_lookup)
        ...
        self.cat_list = [self.user_info_list, self.item_info_list, self.context_list, self.shop_list, self.item_list, self.cate_list, self.aoi_list, self.hour_list]
    ...
    def recieve_gather_features(self, cat_feas, dense_feas):
        ...
        self.user_info_list_emb, self.item_info_list_emb, self.context_list_emb, self.shop_list_emb, self.item_list_emb, self.cate_list_emb, self.aoi_list_emb, self.hour_list_emb = self.ptable_lookup(list_ids=self.cat_fea_split, v_name=self.__class__.__name__)
    ...
    def __call__(self, cat_features, dense_features, auxiliary_info, embed_dim, se_block):
        ...
        # target_emb = tf.reshape(self.item_info_list_emb[:, :2], (-1, 1, 2*embed_dim))
        ...
        target_param = self.ptable_lookup2(list_ids=tf.expand_dims(self.item_info_list[:, 0], 1), v_name=self.__class__.__name__+'lookup_share_tower')[0]
        can_output = self.can_unit(self.my_dense(self.shop_list_emb, embed_dim, 'export_input'), target_param)
        mask = tf.count_nonzero(self.dense_seq_fea, -1, keep_dims=True) # [B, 1]
        mask_seq = tf.sequence_mask(mask, self.seq_len, dtype=tf.float32) # [B, 1, 300]
        can_output = tf.squeeze(tf.matmul(mask_seq, can_output), 1) # [B, 8]

        output = tf.concat([can_output, output_dense, output_din, tf.layers.flatten(self.user_info_list_emb), tf.layers.flatten(self.item_info_list_emb)], -1)
        return output
