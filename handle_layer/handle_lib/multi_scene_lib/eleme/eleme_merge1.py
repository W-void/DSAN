...
class elemeMerge(InputBase):
    def __init__(self, data_struct, params, is_training, hashtable, ptable_lookup):
        super(elemeMerge, self).__init__(data_struct, params, is_training, hashtable, ptable_lookup)
    ...
    def __call__(self, cat_features, dense_features, auxiliary_info, embed_dim, se_block):
        ...
        tgt_scene_list = [tf.expand_dims(self.context_list[:, 2], 1), tf.expand_dims(self.context_list[:, 3], 1)]
        seq_scene_list = [self.time_type_list, self.weekdays_list]
        ...
        tgt_scene_param_merge = tf.stack(tgt_scene_param_list, -1) # [B, 1, param_dim, param_num]
        seq_scene_param_merge = tf.stack(seq_scene_param_list, -1) # [B, seq, param_dim, param_num]

        param_weight = tf.get_variable('param_weight', shape=[len(tgt_scene_param_list), 1], initializer=tf.random_normal_initializer())
        seq_scene_param_merge = tf.squeeze(tf.matmul(seq_scene_param_merge, param_weight), -1) # [B, seq, param_dim]
        tgt_scene_param_merge = tf.squeeze(tf.matmul(tgt_scene_param_merge, param_weight), -1) # [B, 1, param_dim]

        tgt_scene_param_list.append(tgt_scene_param_merge)
        seq_scene_param_list.append(seq_scene_param_merge)

        output_dsan = []
        self.target_brand_emb = self.item_info_list_emb[:, 0]
        self.clk_brand_emb = self.shop_list_emb
        ...
        output = tf.concat(output_dsan + [output_dsan_merge, output_dense, output_din, tf.layers.flatten(self.user_info_list_emb), tf.layers.flatten(self.item_info_list_emb)], -1)
        return output
